...
@Component
public class ReedScraper implements JobScraper {
    ...
    public List<JobPosting> scrape(String keywords, String location, String outputLocation) {
        ...
        try {
            ...
            System.err.println("Error fetching from Reed: " + e.getMessage());
        }
    ...
    private void writeToFile(List<JobPosting> jobs, String keywords, String location, String outputLocation) {
        try {
            String basePath = (outputLocation != null && !outputLocation.isEmpty()) 
                ? outputLocation 
                : outputBasePath;
            String fileName = "reed_" + keywords.replace(" ", "_") + "_" + location.replace(" ", "_") + ".html";
            File outputDir = new File(basePath, "Reed");
            if (!outputDir.exists()) {
                outputDir.mkdirs();
            }
            ...
            System.err.println("Error writing Reed HTML file: " + e.getMessage());
        }
    }
    ...
    private String buildHtml(List<JobPosting> jobs, String keywords, String location) {
        ...
        html.append("<title>Reed Jobs - ").append(escapeHtml(keywords)).append(" - ").append(escapeHtml(location)).append("</title>\n");
        html.append("<style>\n");
    ...
    private List<JobPosting> fetchJobs(String url) throws IOException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful() || response.body() == null) {
                System.err.println("Reed API request failed: " + response.code());
                return jobs;
            }
        ...
        try {
            ...
            for (int i = 0; i < resultsArray.size(); i++) {
                try {
                    ...
                    System.err.println("Error parsing Reed job entry: " + e.getMessage());
                }
            }
        } catch (Exception e) {
            System.err.println("Error parsing Reed JSON: " + e.getMessage());
        }
